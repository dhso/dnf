...
rm -rf /root/DGPServer
rm -rf /root/dungeon.lst
...
chmod +x /TeaEncrypt
DNF_DB_PASSWORD=${DNF_DB_PASSWORD:0:8}
DEC_GAME_PWD=`/TeaEncrypt $DNF_DB_PASSWORD`
echo "game password: $DNF_DB_PASSWORD"
echo "game pwd key: $DEC_GAME_PWD"

# 替换环境变量
find /home/template/neople-tmp -type f -name "*.cfg" -print0 | xargs -0 sed -i "s/GAME_PASSWORD/$DNF_DB_PASSWORD/g"
...
sed -i --follow-symlinks "s/DB_PASSWORD/$DNF_DB_PASSWORD/g" `find /root -name "config.ini"`
...
flush privileges;
grant all privileges on *.* to 'root'@'%' identified by '$DNF_DB_PASSWORD';
grant all privileges on *.* to 'game'@'%' identified by '$DNF_DB_PASSWORD';
flush privileges;
...
mysql -u root -p$DNF_DB_PASSWORD -P 3306 -h 127.0.0.1 <<EOF
